...
def sanitize_raw_content(value, policy: ContextPolicy, warnings: list):
    if value is None: return None
    try:
        decoded=json.loads(value)
    except (TypeError, ValueError):
        return _text(value, policy.max_text, warnings, "raw.content")
    return json.dumps(sanitize(decoded, policy, warnings, "raw.content"), sort_keys=True, separators=(",",":"), ensure_ascii=False)
...
class ContextBuilder:
    ...
    def build(self, org_id: UUID, investigation_id: UUID) -> ContextSnapshot:
        ...
        if promotion:
            ...
            if not cluster or cluster.org_id!=org_id: warnings.append({"code":"PROMOTION_CLUSTER_LINK_MISSING"})
            elif cluster.correlation_version=="correlation-v1": warnings.append({"code":"CORRELATION_V1_CONTEXT_UNSUPPORTED"})
            elif cluster.correlation_version!="correlation-v2": warnings.append({"code":"CORRELATION_VERSION_UNSUPPORTED"})
            else:
                ...
                if not members: warnings.append({"code":"CORRELATION_V2_MEMBERSHIP_MISSING"})
