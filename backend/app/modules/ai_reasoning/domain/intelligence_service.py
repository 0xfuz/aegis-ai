...
class InvestigationIntelligenceService:
 ...
 async def run(self,org:UUID,investigation:UUID)->IntelligenceAnalysis:
  context=self.context(org,investigation); raw=json.dumps(context,sort_keys=True,separators=(",",":")); provider=type(self.llm).__name__.replace("Provider", "").lower(); model=getattr(self.llm,"model",None) or "unknown"; analysis=IntelligenceAnalysis(org_id=org,investigation_id=investigation,provider=provider,model=model,prompt_template_version=PROMPT_VERSION,input_snapshot=context,input_hash=hashlib.sha256(raw.encode()).hexdigest(),status="RUNNING");self.db.add(analysis);self.db.flush()
  try:
   ...
   groups=[("SUMMARY",[Entry(statement=output.summary)]),("OBSERVATION",output.observations),("HYPOTHESIS",output.hypotheses),("RECOMMENDATION",output.recommendations),("QUESTION",output.questions),("REASONING",output.reasoning)]
   ...
   for kind,items in groups:
    for item in items:
     ...
     if kind!="SUMMARY" and not item.supporting_facts: raise ValidationError("AI output contains missing factual aliases.")
     if kind!="SUMMARY": resolve_aliases(fact_aliases,aliases)
   for kind,items in groups:
    for ordinal,item in enumerate(items):
     row=IntelligenceItem(org_id=org,analysis_id=analysis.id,investigation_id=investigation,kind=kind,ordinal=ordinal,statement=item.statement,confidence=item.confidence,payload=item.model_dump(exclude={"statement","confidence","supporting_facts","contradicting_facts"}));self.db.add(row);self.db.flush()
   ...
   analysis.status="COMPLETED";analysis.generated_at=datetime.now(timezone.utc);self.db.commit();return analysis
  except Exception as exc: analysis.status="FAILED";analysis.error_summary=str(exc)[:2000];self.db.commit();raise
 def latest(self,org:UUID,investigation:UUID):
  ...
  return {"id":str(row.id),"status":row.status,"generated_at":row.generated_at,"items":[{"id":str(x.id),"kind":x.kind,"statement":x.statement,"confidence":x.confidence,"payload":x.payload,"review_status":x.review_status,"fact_links":[{"fact_id":str(l.fact_id),"role":l.role} for l in links if l.item_id==x.id]} for x in items]}
 def review(self,org:UUID,item_id:UUID,user:UUID,status:str,rationale:str):
  ...
  allowed={"UNREVIEWED":{"APPROVED","REJECTED"},"APPROVED":{"SUPERSEDED"},"REJECTED":set(),"SUPERSEDED":set()}
  if status not in allowed.get(item.review_status,set()): raise ValidationError(f"Invalid review transition from {item.review_status} to {status}.")
