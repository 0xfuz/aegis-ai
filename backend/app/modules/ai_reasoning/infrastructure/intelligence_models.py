...
class IntelligenceAnalysis(Base, UUIDPrimaryKeyMixin, TimestampMixin, OrgScopedMixin):
    ...
    __table_args__ = (CheckConstraint("status IN ('RUNNING','COMPLETED','FAILED')", name="ck_intelligence_analyses_status"), Index("ix_intelligence_analyses_investigation_generated", "investigation_id", "generated_at"))
...
class IntelligenceItem(Base, UUIDPrimaryKeyMixin, TimestampMixin, OrgScopedMixin):
    ...
    __table_args__ = (CheckConstraint("kind IN ('SUMMARY','OBSERVATION','HYPOTHESIS','RECOMMENDATION','QUESTION','REASONING')", name="ck_intelligence_items_kind"), CheckConstraint("review_status IN ('UNREVIEWED','APPROVED','REJECTED','SUPERSEDED')", name="ck_intelligence_items_review"), Index("ix_intelligence_items_analysis_kind", "analysis_id", "kind"))
