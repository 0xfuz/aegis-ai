...
def test_approved_item_becomes_durable_finding_and_preserves_fact_links(db):
 org,user,inv,entity=setup(db);asyncio.run(InvestigationIntelligenceService(db,Provider()).run(org.id,inv.id));item=db.scalar(select(IntelligenceItem).where(IntelligenceItem.org_id==org.id,IntelligenceItem.kind=="OBSERVATION"));InvestigationIntelligenceService(db).review(org.id,item.id,user.id,"APPROVED","verified");finding=FindingService(db).create_from_item(org.id,item.id,user.id);assert db.get(Finding,UUID(finding["id"])).source_intelligence_item_id==item.id;assert db.scalar(select(FindingFactLink.fact_id).where(FindingFactLink.finding_id==finding["id"]))==entity.id;assert db.scalar(select(IntelligenceFactLink.fact_id).where(IntelligenceFactLink.item_id==item.id))==entity.id;assert db.scalar(select(AuditEvent).where(AuditEvent.action=="FINDING_CREATED_FROM_AI"))
...
def test_http_overview_exact_canonical_counts_and_tenant_isolation(db):
 ...
 headers={"Authorization":f"Bearer {create_access_token(user.id,org.id,'analyst',['investigation:read'])}"};response=TestClient(app).get(f"/api/v1/investigations/{inv.id}/overview",headers=headers);assert response.status_code==200;body=response.json();assert body["investigation"]["id"]==str(inv.id);assert body["counts"]=={"evidence_items":1,"raw_records":1,"events":1,"entities":3,"indicator_occurrences":1,"relationships":1,"findings":0,"confirmed_mitre_mappings":0,"intelligence_runs":0}
def test_http_a6_end_to_end_workflow_and_provenance(db):
 ...
 retired=client.post(f"/api/v1/investigations/intelligence/items/{observation.id}/finding",json={},headers=headers);assert retired.status_code==404
 finding=FindingService(db).create_from_item(org.id,observation.id,user.id)
 persisted=db.get(Finding,UUID(finding["id"]))
 assert persisted.source_intelligence_item_id==observation.id and finding["fact_links"][0]["fact_id"]==str(entity.id)
 found=client.get(f"/api/v1/investigations/{inv.id}/findings",headers=headers);assert found.status_code==200 and found.json()["items"][0]["id"]==finding["id"]
 ...
 overview=client.get(f"/api/v1/investigations/{inv.id}/overview",headers=headers);assert overview.status_code==200 and overview.json()["counts"]["findings"]==1 and overview.json()["counts"]["confirmed_mitre_mappings"]==1 and overview.json()["counts"]["intelligence_runs"]==1
 audit=client.get(f"/api/v1/investigations/{inv.id}/audit",headers=headers);assert audit.status_code==200;actions={row["event_type"] for row in audit.json()["items"]};assert {"FINDING_CREATED_FROM_AI","FINDING_STATUS_CHANGED","MITRE_CONFIRMED","MITRE_REJECTED"}.issubset(actions);assert db.get(Entity,entity.id).canonical_value==entity.canonical_value
