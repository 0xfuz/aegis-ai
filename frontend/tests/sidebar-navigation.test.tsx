...
const state = vi.hoisted(() => ({ pathname: "/investigations", apiFetch: vi.fn() }));
vi.mock("next/navigation", () => ({ usePathname: () => state.pathname }));
vi.mock("next/link", () => ({ default: ({ href, children, ...props }: { href: string; children: ReactNode }) => <a href={href} {...props}>{children}</a> }));
vi.mock("@/lib/auth-context", () => ({ useAuth: () => ({ user: { full_name: "Analyst", role: { name: "analyst" } } }) }));
vi.mock("@/lib/api-client", () => ({ apiFetch: state.apiFetch }));

describe("sidebar navigation", () => {
  beforeEach(() => { state.apiFetch.mockResolvedValue({ open_investigations: 0 }); });
