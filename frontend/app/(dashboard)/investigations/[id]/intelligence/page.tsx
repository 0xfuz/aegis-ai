...
export default function IntelligencePage() {
  ...
  const reconstructionRetry = reconstruction.retry;
  ...
  const onRunCompleted = useCallback(async () => {
    await load();
    reconstructionRetry();
  }, [load, reconstructionRetry]);
  const intelligenceRun = useInvestigationIntelligenceRun(id, onRunCompleted);
  ...
  return <><div className="mb-2 text-xs text-text-muted"><Link href="/investigations">Cases</Link> / Investigation Intelligence</div><h1 className="font-display text-lg">Investigation Intelligence</h1><WorkspaceNav investigationId={id} />
    ...
    <IntelligenceRunProgress run={intelligenceRun.run} submitting={intelligenceRun.submitting} initializing={intelligenceRun.initializing} error={intelligenceRun.error} />
