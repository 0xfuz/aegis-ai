...
function safeCodes(value: ReconstructionJson): string {
  if (!value || typeof value !== "object" || Array.isArray(value)) return "No bounded reason codes returned.";
  return Object.entries(value).filter(([, item]) => typeof item === "string" || typeof item === "number" || typeof item === "boolean").slice(0, 4).map(([key, item]) => `${key}: ${String(item)}`).join(" · ") || "No bounded reason codes returned.";
}
...
export function CitationList({ data, selectedRunId }: { data: InvestigationReconstruction; selectedRunId?: string | null }) {
  const citations = selectedRunId === null ? [] : data.sections.citations;
...
export function ClaimExplanationPanel({ analysis, loadError, citations = [], selectedRunId = null, canReview = false, onReviewed = async () => {} }: { analysis: IntelligenceAnalysisRead | null; loadError: string | null; citations?: ReconstructionCitation[]; selectedRunId?: string | null; canReview?: boolean; onReviewed?: () => Promise<void> }) {
  ...
  return <section aria-labelledby="intelligence-claims" className="mt-5"><h2 id="intelligence-claims" className="mb-2 text-base font-medium">Analyst-reviewed claims</h2><Card>
    ...
    {analysis.items.length > 0 && <ol aria-label="Server-ordered intelligence claims" className="space-y-3">{analysis.items.map((claim) => <li key={claim.id} id={`claim-${claim.id}`} tabIndex={-1} className="rounded border border-hairline p-3 focus:outline focus:outline-2 focus:outline-signal"><div className="flex flex-wrap gap-2 text-xs"><span className="font-medium">{claim.claim_type === "OBSERVATION" && claim.origin === "AI" ? "AI observation · reviewable" : claim.claim_type}</span><span>Origin: {claim.origin}</span><span>Review: {claim.review_status}</span>{claim.confidence !== null && <span>Model-supplied confidence: {claim.confidence}%</span>}</div><p className="mt-2 text-sm">{claim.statement}</p><p className="mt-2 text-xs text-text-muted">{claimSemantics(claim)} {claim.review_status === "CONFIRMED" ? "CONFIRMED is an analyst-reviewed claim status; it does not change claim type or origin." : ""}</p><ClaimCitations claimId={claim.id} citations={citations} /><ClaimReviewControls claim={claim} canReview={canReview} onReviewed={onReviewed} /></li>)}</ol>}
  </Card></section>;
}
